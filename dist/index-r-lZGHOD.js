...
function ee(e, r) {
  ...
  const o = e[0], t = r.nextPart.get(o), l = t ? ee(e.slice(1), t) : void 0;
  if (l)
    return l;
...
function le(e) {
  if (Q.test(e)) {
    const r = Q.exec(e)[1], o = r == null ? void 0 : r.substring(0, r.indexOf(":"));
    if (o)
      return "arbitrary.." + o;
  }
}
function ae(e) {
  ...
  return de(Object.entries(e.classGroups), o).forEach(([n, s]) => {
    _(s, t, n, r);
  }), t;
}
function _(e, r, o, t) {
...
function de(e, r) {
  return r ? e.map(([o, t]) => {
    const l = t.map((n) => typeof n == "string" ? r + n : typeof n == "object" ? Object.fromEntries(Object.entries(n).map(([s, a]) => [r + s, a])) : n);
    return [o, l];
  }) : e;
}
function ue(e) {
  ...
  function l(n, s) {
    o.set(n, s), r++, r > e && (r = 0, t = o, o = /* @__PURE__ */ new Map());
  }
  return {
    get(n) {
      let s = o.get(n);
    ...
    set(n, s) {
      o.has(n) ? o.set(n, s) : l(n, s);
    }
  };
}
const re = "!";
function pe(e) {
  const r = e.separator, o = r.length === 1, t = r[0], l = r.length;
  return function(s) {
    const a = [];
    let u = 0, f = 0, p;
...
function me(e, r) {
  ...
  return e.trim().split(ge).map((s) => {
    ...
    let g = t(p ? f.substring(0, p) : f), h = !!p;
    if (!g) {
      if (!p)
        return {
          isTailwindClass: !1,
...
function ye(e, ...r) {
  ...
  function s(u) {
    ...
    return o = be(f), t = o.cache.get, l = o.cache.set, n = a, a(u);
  }
  function a(u) {
    const f = t(u);
    if (f)
...
const oe = /^\[(?:([a-z-]+):)?(.+)\]$/i, xe = /^\d+\/\d+$/, we = /* @__PURE__ */ new Set(["px", "full", "screen"]), ve = /^(\d+(\.\d+)?)?(xs|sm|md|lg|xl)$/, ke = /\d+(%|px|r?em|[sdl]?v([hwib]|min|max)|pt|pc|in|cm|mm|cap|ch|ex|r?lh|cq(w|h|i|b|min|max))|\b(calc|min|max|clamp)\(.+\)|^0$/, ze = /^-?((\d+)?\.?(\d+)[a-z]+|0)_-?((\d+)?\.?(\d+)[a-z]+|0)/, Ce = /^(url|image|image-set|cross-fade|element|(repeating-)?(linear|radial|conic)-gradient)\(.+\)$/;
function x(e) {
